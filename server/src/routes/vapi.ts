...
// building one would mean slicing raw RAG/focal-entry text, which risks an
// unnatural or garbled greeting with no model in the loop to phrase it. The
// model still gets the FULL context (name/focalEntry/todayContext) for its
// first real reply, so the entry-specific hook still happens, just one turn
// later, spoken naturally rather than templated. See callConfigHandler's
// firstMessage/firstMessageMode override for why this exists.
export function buildFirstMessage(name: string): string {
  const who = name.trim() ? `, ${name.trim()}` : ''
  return `Hi${who}. I'm your private AI journal companion. What's on your mind?`
}
...
export async function callConfigHandler(req: Request, res: Response) {
  ...
  const assistantOverrides: Record<string, unknown> = {
    ...
    // The opening line is spoken by Vapi's TTS directly, NOT generated by our
    // custom-LLM proxy: `assistant-speaks-first` fires it the instant the call
    // connects, with no RAG, no LLM round trip, and no dependence on Vapi's
    // turn-taking/endpointing engine at all. Production logs showed why this
    // matters: when the previous (LLM-generated) opening turn was still
    // in-flight and the user kept talking, Vapi repeatedly abandoned it and
    // opened a NEW turn for the extended transcript (confirmed: ~18% of turns
    // superseded before resolving, one call hit 5 in a row), and each restart
    // pays the full RAG+LLM cost again, stacking to 20-30s before anything was
    // heard. A static firstMessage can't be delayed or superseded by user
    // speech because it never goes through that pipeline. Overrides whatever
    // firstMessageMode the dashboard assistant has (ADR-0153).
...
class SseContentMeter {
  ...
  /**
   * The actual assistant text relayed so far. Used to resolve a turn's
   * `pendingTurn.textPromise` so a coalesced duplicate request gets the exact
   * words the original connection spoke, not just a character count.
   */
  text = ''
...
// Measured production latency (see ADR-0109 + the retry-storm investigation):
// a backchannel-skip turn's LLM TTFB alone runs ~1.4-1.8s, and a turn that
// pays for RAG stacks another ~0.6-1.5s on top, landing at ~2.2-3.3s to first
// real content. Vapi's (undocumented) hang detector reacts to silence within
// ~2-3s by abandoning the request and opening a brand-new one for the same
// unresolved turn. This delay is deliberately BELOW that danger zone but ABOVE
// most single-LLM-TTFB turns: fast turns finish and are cancelled before it
// ever fires (no filler spoken), while the slower RAG-bearing tail still gets
// audio in time to keep Vapi from bailing.
const TURN_FILLER_DELAY_MS = 1_500

// Spoken only when a turn is actually slow (see TURN_FILLER_DELAY_MS above),
// never as a reflexive verbal tic on every turn. Short, complete acknowledgment
// words rather than a disfluency like "um"/"hm": those read as a stutter when
// they're the first thing spoken on every other turn. Rotated by turn sequence
// (not random) so consecutive slow turns in the same call don't repeat the same
// word. NOT a `finish_reason` chunk: the stream stays open and the real content
// follows in the same SSE response.
const TURN_FILLER_PHRASES = ['Got it. ', 'I see. ', 'Right. ', 'Sure. ']

function writeFillerChunk(res: Response, text: string): void {
  const chunk = {
    id: 'chatcmpl-filler',
    object: 'chat.completion.chunk',
 ...
 */
const TURN_FAILED_LINE = "Sorry, I'm having a moment, could you say that again?"

export async function llmProxyHandler(req: Request, res: Response, database = db): Promise<void> {
  // Set once we register this turn as the session's in-flight/cached generation,
  // so the outer catch can still unblock any joined duplicate if something
  // throws unexpectedly after registration.
  let resolveTurnText: ((text: string) => void) | undefined
  // Set once the SSE stream opens, so the outer catch can cancel a pending
  // filler timer if something throws unexpectedly after that point.
  ...
  try {
    ...
    // Fingerprints an unresolved turn by its exact replayed history. Vapi retries
    // an unresolved turn by sending the SAME messages array again (see the
    // retry-storm dedup below), so an unchanged fingerprint means "this is a
    // retry of a turn we've already seen," not a new user turn.
    const turnFingerprint = JSON.stringify(inbound)
    ...
    // Open the SSE stream. A filler line fires ONLY if the turn is still silent
    // after TURN_FILLER_DELAY_MS (see constant above): most turns finish first
    // and this timer is cancelled, unspoken. `cancelFiller()` MUST be called on
    // every exit path below before writing real/failure content, or a late timer
    // firing after `res.end()` would write to an already-closed stream.
    res.setHeader('Content-Type', 'text/event-stream')
    res.setHeader('Cache-Control', 'no-cache')
    res.setHeader('Connection', 'keep-alive')
    res.flushHeaders()
    const fillerTimer = setTimeout(() => {
      writeFillerChunk(res, TURN_FILLER_PHRASES[(seq - 1) % TURN_FILLER_PHRASES.length])
    }, TURN_FILLER_DELAY_MS)
    cancelFiller = () => clearTimeout(fillerTimer)

    // DIAGNOSTIC (retry-storm investigation). Confirms whether Vapi actually
    // abandons/closes an earlier request's socket when it opens a retry for the
    // same turn, or leaves it open. `req.on` is absent on plain test doubles, so
    // this is guarded rather than assumed.
    if (typeof (req as any).on === 'function') {
      (req as any).on('close', () => {
        console.log(`[vapi/llm] connection closed chat=${chatTag} seq=${seq}`)
      })
    }

    // Retry-storm dedup: Vapi's custom-LLM connector can decide a turn is
    // unresolved and open a BRAND NEW request carrying the identical `messages`
    // history rather than waiting on the one already in flight. Previously every
    // one of those retries re-ran RAG and paid for a fresh, independent (and
    // non-deterministic) completion: up to 9 in production for a single turn.
    // A matching fingerprint means this IS that retry: join the existing
    // generation's promise instead of starting over. The promise is left on the
    // session after it resolves, so a LATE duplicate arriving after the turn
    // already completed still gets the identical cached answer.
    if (session.pendingTurn?.fingerprint === turnFingerprint) {
      console.log(`[vapi/llm] turn DEDUPED chat=${chatTag} seq=${seq}, joining in-flight/cached generation`)
      const text = await session.pendingTurn.textPromise
      cancelFiller()
      writeGracefulCompletion(res, text)
      return
    }
    let resolved = false
    const textPromise = new Promise<string>(resolve => {
      resolveTurnText = (text: string) => {
        if (resolved) return
        resolved = true
        resolve(text)
      }
    })
    session.pendingTurn = { fingerprint: turnFingerprint, textPromise }
    ...
    // A call's opening turn arrives with NO prior user/assistant history, which
    // would leave `messages` as system-only. Venice's OpenAI→Gemini translation
    // drops a system-only payload into an EMPTY `contents` array, which Vertex
    // rejects with 400 "at least one contents field is required" (confirmed in
    // production logs). Seed a neutral trigger turn so `contents` is never empty;
    // PROMPTS.voiceChat's OPENING instruction tells the model how to use it.
    if (history.length === 0) history.push({ role: 'user', content: '(call connected)' })
    ...
    try {
      ...
      cancelFiller()
      resolveTurnText!(TURN_FAILED_LINE)
      writeGracefulCompletion(res, TURN_FAILED_LINE)
      return
    }
    ...
    // A real, ok streaming response is imminent: cancel the filler timer now
    // rather than waiting for the first byte, so it can't fire mid-relay.
    cancelFiller()
    ...
    try {
      ...
      resolveTurnText!(meter.text || TURN_FAILED_LINE)
      try { res.end() } catch {}
    }
  } catch (err) {
