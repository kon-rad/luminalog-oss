...
describe('buildSoulPayload', () => {
  ...
  it('returns the point-set and stats, exposing no vectors', async () => {
    ...
    userData['u1'] = { stats: { streakCount: 5, maxStreakCount: 9, totalWords: 12345, goalDayWords: 800 } }
    const payload = await buildSoulPayload('u1')
    expect(payload.constellation.version).toBe(3)
    expect(payload.stats).toEqual({ streakCount: 5, maxStreakCount: 9, totalWords: 12345, goalDayWords: 800 })
