...
/// text → different vector), correctly shaped (512-dim, L2-normalized), and needs no
...
/// distiluse ONNX model is hosted (increment 1c-D).
///
/// Pure and side-effect-free: no I/O, no global state, no `Date()`.
struct StubTextEmbedder: TextEmbedder {

    /// Output dimension. Defaults to the shipping model's 512 so the stub is a
