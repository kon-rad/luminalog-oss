...
/// implementation runs EmbeddingGemma via ONNX Runtime (`ONNXTextEmbedder`); the
...
/// `EmbeddingVector.dimension` (768), so callers can feed it straight into
/// `VectorIndex` / `EncryptedVectorStore`.
protocol TextEmbedder {
    /// Embed a single string into a normalized 768-dim vector.
