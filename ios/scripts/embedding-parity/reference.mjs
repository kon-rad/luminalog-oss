...
//
// Produces reference EmbeddingGemma vectors for the shared `texts.json` using
...
// reference BEFORE any real vector is written to the encrypted store (the 768-dim and
...
const MODEL_ID = process.env.EMBEDDING_MODEL_ID ?? "onnx-community/embeddinggemma-300m-ONNX";
...
for (let i = 0; i < texts.length; i++) {
  ...
  if (vec.length !== 768) {
    throw new Error(`Expected 768 dims, got ${vec.length} for text #${i}. ` +
...
writeFileSync(outPath, JSON.stringify({ model: MODEL_ID, dimension: 768, texts, vectors }));
