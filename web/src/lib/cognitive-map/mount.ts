...
export function mountCognitiveMap(
  ...
  const distance = (): number => {
    // Array.from rather than spreading the iterator: this file is compiled by three
    // different toolchains (this package, the Next build, esbuild for iOS), and the
    // Next build has no `target` set, so it defaults to ES5 where spreading a
    // MapIterator is a hard compile error.
